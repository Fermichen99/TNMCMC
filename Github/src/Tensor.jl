...
@inline function _superspin_bond_cached(pa::Parament, sp::Spin,
    ...
    return _tl_get!(cache, key) do
        m = pa.BondTensor[bond_idx_vec[1]]
        for k in 2:pa.zchunk
            m = kron(m, pa.BondTensor[bond_idx_vec[k]])
        end
        m
    end
end
...
function OutTensorGrid(pa::Parament, sp::Spin, te::Tensor,
    ...
    @inbounds for tey in 1:pa.ychunk, tex in 1:pa.xchunk
        ...
        copy_tensor = _copy_tensor_cached(pa, te, h0s, h1s, Js)
        bonds = ntuple(i -> _superspin_bond_cached(pa, sp, zin, lay, lax, nnb_list[i]), 3)
...
function InitTensor(zin::Int, yin::Int, xin::Int,pa::Parament,sp::Spin, spin::Array{Int,3},te::Tensor)
    StoreTensor = OutTensorGrid(pa, sp, te, spin,zin,yin,xin)
...
function Contruction(pa::Parament, te::Tensor, J::Array{Int}, mps::Array{T,S}) where {T,S}
    ...
    for i in len:-1:2
        mps[i-1] = ein"ijk,k,lin->ljn"(mps[i],Out_superspin_Field(J[:,i],pa),mps[i-1])
        tnorm = norm(mps[i-1])
        mps[i-1] /= tnorm
...
function Out_superspin_Field(ju::Array{Int,1},pa::Parament)
    field_dim = length(ju)
    if pa.zchunk == 0
    ...
    elseif field_dim == 1 # No Kronecker product needed, just get the 2x2 matrix
        return ju[1] == 1 ? pa.field :  ju[1] == -1 ? pa.Antifield : ones(2)
    end

    field_matrices = Vector{Any}(undef, field_dim)
    for i in 1:field_dim
        field_matrices[i] = ju[i] == 1 ? pa.field :  ju[i] == -1 ? pa.Antifield : ones(2)
    end
...
"""
function TNMH_oe(pa::Parament, sp::Spin, te::Tensor,spin::Array{Int,3},zin::Int,yin::Int,xin::Int)
...
function Accept(pa::Parament,sp::Spin,st::Statistics, da::DataFile, spinold::Array{Int,3},spinnew::Array{Int,3},lnprold::Float64,lnprnew::Float64,Energyold::Float64,Energynew::Float64,prt::Bool=false)
    deltaE = pa.Beta*(Energynew-Energyold)
...
function select_zeros!(zin::Int, yin::Int,xin::Int,zlen::Int,ylen::Int,xlen::Int,i::Int,spin::Array{Int,4})
    spin[zin:zin+zlen-1,yin:yin+ylen-1,xin:xin+xlen-1,i] .= 0
end

function flip_model!(sp::Spin, spin::Array{Int,4})
    new_spin = similar(spin)
    new_bond = similar(sp.bond)
    # the index here need to be flip inverse
    for z in 1:size(spin, 1)
        for y in 1:size(spin, 2)
            for x in 1:size(spin, 3)
